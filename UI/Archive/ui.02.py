...
def animate(i):
    ...
    dataSource = '/home/ros/Desktop/LappyStream/out/Lappy_Sensor_log.txt'
    with open(dataSource) as f:
        num_lines2 = sum(1 for line in f)
    #         print(str(num_lines2))
    if num_lines2 > 21:
    ...
    reader = csv.reader(beginning,delimiter='&')
    for row in reader:
        centerX.append(float(row[0][10:17]) )
        centerY.append(float(row[0][19:26]) )
        boxWidth.append(float(row[1][7:13]) )
        boxHeight.append(float(row[1][16:23]) )
    print(' centerX[19] : ' + str(  centerX[19]  ) + ' centerY[19]: ' + str(  centerY[19] )+ ' boxWidth[19] : ' + str(  boxWidth[19]  ) + ' boxHeight[19]: ' + str(  boxHeight[19]    ))
## Make boggie offsets
    for itr in range(0,19):
        ...
        bogieLat.append(x[itr] + bogieDistance * cos((bogieHeading/180)*math.pi))
        bogieLong.append(y[itr] + bogieDistance * sin((bogieHeading/180)*math.pi))
